...
def main():
    ...
    fig, (ax1, ax2) = plt.subplots(1,2, figsize=(3.4, 2.6), dpi=300)
    ...
    # Legend at top inside axes to avoid header collision — compact, not outside
    ax1.legend(frameon=True, loc="upper left", fontsize=6, handlelength=1.0, borderpad=0.3, labelspacing=0.2, handletextpad=0.4)
    ax1.grid(alpha=0.25, linewidth=0.5); ax1.set_title("Reject Rates", fontsize=7.5, pad=8)
    ...
    ax2.legend(frameon=True, loc="upper left", fontsize=6, handlelength=1.0, borderpad=0.3, labelspacing=0.2, handletextpad=0.4)
    ax2.grid(alpha=0.25, linewidth=0.5); ax2.set_title("Downstream F1", fontsize=7.5, pad=8)
    fig.suptitle("SQI Gate Ablation (n=918, thr=0.35)", fontsize=8, y=0.98, weight="bold")
